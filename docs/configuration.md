...
| `TEAMCLAUDE_HEADERLESS_429_RETRY_DELAY_MS` | `2000` | How long to wait before the single retry of a **headerless** (request-scoped) 429 — one carrying no `retry-after` and no `anthropic-ratelimit-*` headers. Retried once per request — after a failover hop it stays on the account the hop landed on — and only for a caller that waits on the connection (a Codex caller is answered at once instead); if the retry is refused too the 429 goes to the client, just later. Lower it only if you would rather have the 429 sooner than have the transient absorbed — see [the two kinds of 429](routing.md#the-two-kinds-of-429) |
