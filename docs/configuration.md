...
| `accounts[].maxSpend` | Hard per-account **money** cap, in the account's billing currency (`20` is $20.00): once upstream's month-to-date extra-usage figure reaches it, the account receives **no** requests — rotation, the revalidation probe and pins all skip it — until the figure resets with the month. `0` means not one cent. Judged only while extra usage is enabled upstream; a usage cap of `1.0` alongside it stops the account before billing can start at all. Applied live on reload. See [Per-account spend caps](quota.md#per-account-spend-caps) |
