...
| `warmupSchedule` | Optional reset-target keep-warm schedule: `{ "resetTime": "15:30", "timezone": "Europe/Moscow" }`. The timezone must be an IANA name. Mutually exclusive with `warmupSeconds`; set with `teamclaude warmup reset HH:MM --timezone Area/City` |
