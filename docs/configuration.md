...
| `accounts[].allowExtraUsage` | Set `true` to let this account keep serving **past its quota on Anthropic's paid "extra usage" (overage) — this spends real money**. Off by default; only a literal `true` opts in. Rotation is unchanged: the account still rotates away at `switchThreshold` like any other. Only when *no* account can serve under the normal rules does the fallback run, and it spends free quota first: any account between its threshold and 100% serves before an opted-in account is billed. It returns to normal as soon as any account has headroom again. Every hard gate still applies — `disabled`, `maxUsage`, a live 429 hold, an error state, an entitlement cooldown, routes. `maxUsage` above 1.0 (e.g. `1.5`) makes a natural spend limit. Applied live on reload. See [Extra-usage fallback](quota.md#extra-usage-fallback) |
