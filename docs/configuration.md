...
At the 500-value cap that means about 120 KiB when the values churn — one busy day each — against roughly 105 KiB of lifetime rows alone, and about 6.8 MiB when all 500 are genuinely active at once (the file is pretty-printed, so these are bytes on disk). The write is synchronous: serialising that 6.8 MiB case takes ~19 ms before the `fsync`, once a minute. A fleet with a handful of client keys is unaffected either way, but a deployment pointing a high-cardinality dimension at this on slow storage should know the shape of it.
