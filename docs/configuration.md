...
Volatile runtime state (observed quota, per-client and per-dimension usage) is written separately to `teamclaude.state.json` alongside the config, so the config file stays clean and hand-editable. The state file is safe to delete — quota is simply re-learned from traffic, and usage windows refill from the next day's traffic (the lifetime counters do not come back).

That file is small for a typical fleet, but it is no longer trivially small: usage is kept per 15-minute slot for a day (see [Usage windows](usage.md#status-dashboard-browser)). Every client key and dimension value ever seen keeps a row of lifetime counters, as it always has, and those are never deleted; what the day of slots adds is carried only by the keys **active in the last 24 hours**, because a key that goes quiet drops its slots the next time it is read or written. So a dimension that merely accumulates stale values does not accumulate slots with them.
...
| `messageThreads` | Set `true` when the global `upstream` is a relay that keeps Anthropic message-thread state. Accounts without an `upstream` of their own go wherever this points, and when that is a host other than Anthropic's, `thread: {"type": "continue"}` is refused for them the same way it is for a per-account third-party upstream (see `accounts[].messageThreads`); this flag is the fleet-wide way to say the relay does keep threads. Default off. See [message threads](accounts.md#message-threads) |
| `switchThreshold` | Quota utilization (0–1) at which to switch accounts (`teamclaude threshold <1-100>`, or the TUI settings screen: **Switch threshold**). The screen accepts tenths of a percent, e.g. `99.5`, which is stored as `0.995`. Reported OAuth utilization arrives on a whole-percent grid, so a fraction only changes the outcome for an API-key account, whose used share is continuous When any account is opted into `allowExtraUsage`, the fallback spends quota between this threshold and 100% before any account is billed; a reserve that must hold is `maxUsage`. |
...
| `holdSeconds` | Maximum seconds to [hold the connection](quota.md#hold-on-exhaustion) when all accounts are exhausted, polling silently until one recovers (`0` = return 429 immediately, the default). `teamclaude run` raises `API_TIMEOUT_MS` automatically to match. Read at startup — a change needs a restart An `allowExtraUsage` account, if any, serves before the hold engages. |
...
| `quotaBarPercent` | Draw the utilization percentage beside the reset countdown on each TUI quota bar — `97% · 2h30m` — on bars wide enough for both (`true`, the default). `false` keeps the countdown alone, which is how the bars read before the percentage was added. Display only, and only in the TUI: `teamclaude status` and the dashboard carry their own percentage regardless. Toggled from the settings screen (**g** → Bar percentage); applied live on config reload |
...
| `autoRedeemResets` | Let a Codex account whose weekly window has run out spend one of its [free rate-limit reset credits](accounts.md#free-rate-limit-reset-credits) to clear it. **`false` by default**, and deliberately so: a redemption cannot be undone and the credits are scarce, so nothing spends one until you switch this on. The switch is **fleet-wide** because the policy is — a credit is spent only when the weekly (never the 5-hour) window is exhausted **and** either every other Codex account is unavailable too, or the credit itself expires within three days. Toggled from the settings screen (**g** → Auto-redeem) and applied live on reload, so it can be armed or killed without a restart. Individual accounts can opt out with `accounts[].autoRedeemReset` below |
...
| `accounts[].maxSpend` | Hard per-account **money** cap, in the account's billing currency (`20` is $20.00): once upstream's month-to-date extra-usage figure reaches it, the account receives **no** requests — rotation, the revalidation probe and pins all skip it — until the figure resets with the month. `0` means not one cent. Judged only while extra usage is enabled upstream. **Requires the quota probe** (`quotaProbeSeconds`, or `teamclaude probe N`): the spend figure comes only from probe readings, so the cap binds within one probe interval of the figure being reached, not on the request in flight — a usage cap of `1.0` alongside it stops the account before billing can start at all. Applied live on reload. See [Per-account spend caps](quota.md#per-account-spend-caps) |
| `accounts[].allowExtraUsage` | Set `true` to let this account keep serving **past its quota on Anthropic's paid "extra usage" (overage) — this spends real money**. Off by default; only a literal `true` opts in, and **Anthropic OAuth accounts only**: on an API key, a third-party backend (`upstream`) or a Codex login the flag is ignored, with one warning when the account is loaded. Rotation is unchanged: the account still rotates away at `switchThreshold` like any other. Only when *no* account can serve under the normal rules does the fallback run, and it spends free quota first: any account between its threshold and 100% serves before an opted-in account is billed. It returns to normal as soon as any account has headroom again. Every hard gate still applies — `disabled`, `maxUsage`, a live 429 hold, an error state, an entitlement cooldown, routes. `maxUsage` above 1.0 (e.g. `1.5`) makes a natural spend limit, and `accounts[].maxSpend` above caps the month's bill in money. Applied live on reload. See [Extra-usage fallback](quota.md#extra-usage-fallback) |
| `accounts[].routing` | The account's own egress proxy: `"socks5h://user:pass@host:1080"`, also `socks5`, `socks4a`, `socks4` and `http`, and a bare `host:port` is `http`. Every connection made for the account goes through it (requests, sign-in, token refresh, profile, usage and quota), and the account then ignores `upstreamProxy` and sx.org. Set it with `teamclaude routing <name> <url>`, `login --routing`, the TUI (**`g`** then **Account proxy**) or by hand; applied live on reload. See [Per-account routing](accounts.md#per-account-routing-routing) |
| `accounts[].autoRedeemReset` | Set `false` to exempt this **Codex** account from `autoRedeemResets` above: it never spends a reset credit, however dry the pool gets. **Negative only** — `true` and an absent key mean the same thing, and neither arms anything on its own; the fleet-wide `autoRedeemResets` is the only switch that does. Ignored for non-Codex accounts. Applied live on reload |
...
| `accounts[].messageThreads` | Set `true` when this upstream keeps Anthropic message-thread state. Off by default for accounts whose effective upstream — their own `upstream`, or the global one — has a host that is not Anthropic's own (a region pin or mirror needs no flag): a backend that keeps no thread state would be handed only the turn's delta, so the proxy answers `thread: {"type": "continue"}` with the same `400` Anthropic gives for a thread it cannot continue, and Claude Code resends the whole conversation and stops threading that model for the session. Completions only — `count_tokens` is always forwarded. For accounts on the global `upstream`, the top-level `messageThreads` declares it for the whole fleet. See [message threads](accounts.md#message-threads) |
| `accounts[].stripRequestFields` | Array of **top-level** request-body fields to drop before forwarding to this account. For third-party upstreams that implement the Anthropic message API but reject fields Claude Code sends (e.g. `["context_management"]`, which some return a `400 Extra inputs are not permitted` for — breaking every request once that account is selected). Applies to this account only; Anthropic accounts are untouched. Two nested forms are accepted. `"cache_control.<subfield>"` (e.g. `["cache_control.scope"]`) drops that subfield from every documented prompt-cache breakpoint (root, `system[]`, `messages[].content[]`, `tools[]`) for backends that reject it with `400 unknown parameter`; `type` and `ttl` are forwarded unless listed. `"content.<block type>"` (e.g. `["content.tool_addition", "content.tool_removal"]`) drops every `messages[].content[]` block of that type, for backends that answer `400` to the blocks Claude Code uses to announce a tool that appears mid-conversation; such a block stays in the history, so without the entry every later turn of that conversation fails as well. A cache breakpoint on a dropped block moves to the last block left in the same message, and a message left empty is dropped. Nothing is dropped without an entry |
