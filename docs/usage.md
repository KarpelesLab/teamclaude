...
A **warning banner** sits at the top of the page and is empty unless something is wrong. It reports a session that is making requests and getting nothing back (upstream never returned a usage report for it — the case that reads as zero on every token column, exactly like an idle session, and is otherwise invisible), a fleet where no account can serve an unrouted request, an account that needs a person (a broken token, an organization refusal, a disabled entry), and an account that is billing real money. A spent quota bucket or a rate-limit back-off is **not** reported: that is rotation working, and a banner that is always on is one nobody reads.
