...
A **Routing** table above the accounts shows, for each model family the fleet meters separately (Fable, Sonnet, or any configured route), which account a request for it lands on right now and how many accounts could serve it — the ones that cannot are struck through, which is the reason the family is elsewhere. The last row is everything else, which goes to the current account. The target is the server's own answer (`routes[].target` in `/teamclaude/status`), not something the page derives from the quota bars.
