...
**A rollover is measured against what the last request read where it came to rest**, and that is the whole of the state involved: one reading per pin (and one for the sticky current account), taken when a request arrives to find the pin already on an account. A selection that *sends* a request somewhere takes no reading it could lose one by: it writes only where there is nothing to lose — a choice that has never named an account, or one whose account has rolled no window it recorded — because the request may not arrive. And the roll a preemption pushes traffic off is **held** rather than dropped until the stay is confirmed, which one thing does: an attempt that selected under the reading this move stamped is **served** at the destination, meaning upstream answered it with a status below 400. That status comes from the response headers, and a body that fails after them does not un-serve the attempt: the roll is already released by the time the proxy destroys the client's half-written response, so a fail-back after such a failure finds the origin settled and reads that account whole. The reading for the sticky current account is one slot every provider shares, so a success settles only a roll pushed off an account of its own provider: a request whose provider does not own the cursor borrows it and is restored away again before its response is read, and its success is a stay under its own fleet's placement rather than under the one the roll is held for. That sharing also moves the reading: a borrowed walk leaves it naming the borrower's account, so the cursor-owning provider's next request takes a fresh reading of the account it returns to and the rollover test compares that reading against itself. A roll on the account the cursor names therefore goes undetected whenever the reading is naming somewhere else by the time the owner's next request arrives, whether the borrow that moved it fell before that roll or after — though only the rest a borrower takes on a cursor of its own will move a reading whose account has already rolled, so a fleet with no cursor of its own yet leaves a standing roll where it is and the owner preempts on it as usual. That rest is also what puts the roll it carries off into the hold, so only the later order leaves anything to recover: where the borrow followed the roll, a fail-back finds it there and is handed it back, while a reading a borrow had already moved away holds nothing when the roll comes and loses it outright. So a preemption whose destination refuses it, and which then falls back onto the account it was pushed off, finds the rollover still there and moves again as soon as anywhere better is available. Arriving is not being served, and that is what keeps the hold through a retry that never leaves the destination, a 401 re-entering selection on the same account and a short-wait 429 with no idle sibling to fail over to alike — however many requests start there, none of them has been answered. The confirming attempt need not belong to a different request from the one whose selection made the move: the stamp is re-read before every attempt selects, so a retry of that same request can carry the new reading and confirm it. Only once some attempt has already come to rest at the destination, though — the move itself writes nothing there, so the first attempt to arrive is the one that takes the reading, and it selected before the stamp that reading carries. The attempt that made the move carries an older one still and confirms nothing either. The bound that follows is worth stating plainly: **a rollover that happens before anything has read that account is not detected.** That window is one request wide and it is the same one every first placement has — a session's opening account, a rotation's destination, an operator's switch — so in practice it costs the account's *next* roll rather than this one. Holding the origin's roll until the destination serves something has a bound of its own and it is the mirror of that one: traffic that returns to the account that rolled before any request has been served at the destination is preempted off it once more, because the stay was never confirmed. Traffic that returns after something **has** been served there finds the roll already released, and reads the origin whole again: it takes the week that account gained, and the fleet stays on it until that window rolls again. A request still in flight when a later one confirms the stay is where that bites — the confirmation is evidence about the stay its own attempt selected under, and nothing in the reading says an earlier attempt is still outstanding and may yet come back. Where a learned scoped window shares a pin with the bucket it falls under (see above), that one turn is paid by every family on the pin, not only by the family whose window rolled. A rollover that moves nothing logs a line saying which of the two reasons it was — no eligible account could take the traffic, or the re-rank ran and the account it was already on still ranked best — because a stuck preemption otherwise looks exactly like no rollover at all, and like the ordering simply agreeing that staying is right.
...
Four surfaces meet an expired 5-hour window before any request does, and they do not answer alike. A **status read** clears the window — through a sweep that cannot switch, and through the route preview's availability reads — and never reaches the switch at all, knob off or on. A **TUI paint**, the **quota poll** behind the status line, and the **opening placement** at daemon launch call the combined clear-and-switch, so with the knob **off** they take the switch themselves: with no request in hand, and therefore on the model-less ranking, where a request's own refresh ranks on the window that governs it. Launch then places the cursor outright, so its switch settles nothing either way. With the knob **on** none of them takes it, because a reset is spent by a request and none of these is one. The event is deferred, not lost: it stays recorded on the account, and the first request that can be sent both there and to the account the cursor names acts on it, ranked on the window that governs that request. That admission is on the request's main model alone: a request that also names an advisor model is admitted on the main one, so it can spend the event onto an account that cannot serve its advisor and then be diverted away from it for that one request. The cursor is left where a request carrying no advisor would have put it, and the next such request is served there. That deferral lives only in memory: the flag is no part of the quota state the proxy saves, and the poll that raised it has already cleared the window it came from, so a restart in between drops the event outright. Startup then places the traffic by the ranking any start uses, owed nothing — the same fresh placement a restart makes of the account the fleet was on, which is not saved either.
