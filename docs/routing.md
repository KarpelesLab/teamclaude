...
One limitation is **pre-existing and unchanged by this feature**, recorded here because it is easy to attribute to it: a status preview or TUI paint that runs before a request clears an expired 5-hour window, and the account switch that reset would have caused is skipped, because the switch acts on the resets its own pass observed. That is true with the knob off and on alike, and it predates expiry routing.
