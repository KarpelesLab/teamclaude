...
// `expiry` is passed through exactly as given, including absent — the flag-off
// comparisons below depend on the config key genuinely not being there, and a
// default parameter would quietly turn `undefined` back into the knob's ON
// value. That mistake made every off-case in this file assert against a manager
// that had the feature enabled.
...
// Set a weekly bucket: `used` fraction spent, window resetting in `hours`.
// Both halves together, which is the pairing pressure depends on.
//
// `base` exists because pressure is a function of the instant it is read at, and
// a test that compares two readings must take them against ONE clock. Left to
// default, the fixture's `Date.now()` and each `_expiryPressure` call read the
// wall clock separately, so a millisecond passing between them shifts the value
// by far more than an exact comparison tolerates. That made two tests here fail
// intermittently, which a mutation run caught by intersecting two passes.
...
test('an account whose window nobody has reported ranks in the top band', () => {
  // Its reset is the furthest out in the fleet, so the pre-existing tiebreak
  // ranks it last. Unknown pressure ranks it first: using it is how the quota
  // becomes known, and ranking it last would make the unknown permanent.
  const build = expiry => {
...
test('a KNOWN-SPENT account with no clock does not outrank measured expiring quota', () => {
  ...
  // here. Ranked as pure discovery, turning the feature ON inverted this case:
  // the account 95% through its Fable quota beat the one holding 95% of it with
  // an hour to go, which is the feature's own goal running backwards.
  const build = expiry => {
  ...
  // Still admitted, and still published as unknown on the wire: only its RANK
  // changed. Being used is how the missing window gets reported.
...
test('a clockless account still outranks measured quota it genuinely beats', () => {
  // The other direction, so the fix orders rather than demotes: an account
...
test('path 3: a band member with strictly worse pressure is still refused', () => {
  ...
  // no longer stops it. The rank comparison is the guard that does, and the two
