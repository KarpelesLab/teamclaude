...
test('a known utilization with no clock carries the least it can be worth', () => {
  // The window resets at most a week out, so scoring it as though it were the
  // furthest it could be is a lower bound: the account can only turn out to be
  // worth more than this, never less.
  ...
  // Overage clamps to nothing left, exactly as a measured pressure does.
  assert.equal(bound(1.4), 0);
  // The three reasons that know nothing carry no bound at all.
...
test('a malformed utilization is absent, never clamped into a perfect score', () => {
  // The clamp would read NaN as 0 — a completely unspent window, the strongest
  // score there is, invented out of a value that arrived broken.
...
test('a window whose reset has passed is a known pressure of zero, not an absence', () => {
  // Known to be worth nothing to spend for expiry reasons is a different claim
  // from knowing nothing about it, and only the second earns a top-band place.
...
test('pressureRank sorts higher pressure first, and discovery at the very front', () => {
  ...
  // Nothing known: outranks every known value, because being used is how the
  // quota is learned. Ranking it last would make the unknown permanent.
...
test('a known-but-clockless account does NOT outrank a measured one it is worse than', () => {
  // The failure this exists to prevent: `no-reset` ranked as discovery put an
  // account known to be 95% spent ahead of one known to be holding 95% of its
  // window with an hour left, and `-Infinity` wins outright so no later
  // tiebreak could recover it.
...
test('a clockless account still outranks a measured one it is better than', () => {
  // The other direction, so the fix is a correct ordering rather than a demotion:
  // holding almost the whole window is worth more than a measured trickle, even
  // scored against the most pessimistic horizon a weekly window can have.
...
test('an account with no comparable pressure stays in the band', () => {
  // Banding it out would make the unknown permanent: using it is how the
  // governing window gets reported in the first place.
...
test('a known-but-clockless account is admitted too, though it no longer ranks top', () => {
  // ADMISSION is unchanged by the ranking fix, and deliberately so: using the
  // account is still how its window gets reported. Only the order changed, and
