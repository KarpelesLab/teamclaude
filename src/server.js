...
const SERVING_ACCOUNT_HEADER = 'x-teamclaude-account';
...
function encodeServingAccount(name) {
  return encodeURIComponent(String(name).normalize('NFC').toWellFormed());
}
...
export async function forwardRequest(req, res, body, accountManager, upstream, retryCount, hooks, reqId, ctx, logDir, sx, useSx) {
  ...
  try {
    ...
    // Override any upstream value so attribution always names the account whose
    // terminal response this proxy actually relays. URI encoding keeps arbitrary
    // configured display names safe as an HTTP header value.
    responseHeaders[SERVING_ACCOUNT_HEADER] = encodeServingAccount(account.name);
